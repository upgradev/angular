    ...
    register: async function (req, res) {
        try {
            ...
            if (!u) {
                ...
                res.status(403).json({ message: 'E-Mail already registered', error: error })
            }

        } catch (error) {
            res.status(500).json({ message: 'Error while save the user', error: error })
        }
    },
        ...
        jwt.verify(token, consts.keyJWT,
            (err, decoded) => {
                ...
                UserModel.findById(id).lean().exec(function(err, user){
                    if(err|| !user){
                        return res.status(500).json(
                            {message: 'Error triyng to fetch user data ', error: err})
                    }
                    let token = jwt.sign(
                        { _id: user._id }, consts.keyJWT,
                        { expiresIn: consts.expiresJWT }
